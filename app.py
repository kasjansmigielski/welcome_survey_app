    ...
    if st.session_state['first_button_pressed']:
            #WCZYTANIE DANYCH
                ...
                with st.sidebar:
                    ...
                    # Pobieramy ścieżkę od użytkownika
                    audio_path = st.text_input('Wpisz ścieżkę do pliku MP3')
                    if audio_path == '':
                        st.error('Nie wpisano żadnej ścieżki audio')
                    else:
                        PATH = Path(audio_path)
                        if not PATH.exists():
                            st.error('Nieprawidłowa ścieżka audio')
                        else:
                            st.session_state['audio_path'] = PATH
                            st.markdown("### :musical_note: Tło muzyczne :musical_note:")
                            st.audio(PATH.name)
                ...
                if st.session_state['second_button_pressed']:
                    ...
                    #menu główne
                    st.markdown("""
                    <p style="color:red; font-size:24px;">Znajdujesz się w menu głównym aplikacji</p>
                    ...
                    st.markdown("""
                                ...
                                * doświadczenie zawodowe i jaka branża.
                                """)
                    st.markdown(':arrow_left: Po lewej stronie znajdują się wszystkie dostępne filtry oraz dostępne tło dźwiękowe. :notes:')
